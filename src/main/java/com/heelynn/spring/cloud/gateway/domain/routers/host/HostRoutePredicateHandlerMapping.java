...
/**
 * @author heelynn
 * @date 2024/8/27 16:18
 *
 * 从{@link RoutePredicateHandlerMapping} 源码中完全copy过来，代码基本完全一致
 * 但仍然继承{@link RoutePredicateHandlerMapping}的原因是：需要替代Spring根据类型自动装填{@link RoutePredicateHandlerMapping}的Bean
 *
 * 修改方法 {@link HostRoutePredicateHandlerMapping#lookupRoute(ServerWebExchange)}
 * 让其根据Host选择Route
 */
public class HostRoutePredicateHandlerMapping extends RoutePredicateHandlerMapping {
    ...
    public HostRoutePredicateHandlerMapping(FilteringWebHandler webHandler, RouteLocator routeLocator,
    ...
    protected Mono<Route> lookupRoute(ServerWebExchange exchange) {
        // 默认从routeLocator中获取routes
        Flux<Route> routes = routeLocator.getRoutes();
        // 如果routeLocator实现了ICachingRouteLocatorCustom接口，则调用getRootRoutes(exchange)方法获取routes
        // getRoutes 会根据exchange中的某项信息作为key获取缓存的routes
        if(ICachingRouteLocatorCustom.class.isAssignableFrom(routeLocator.getClass())){
            routes = ((ICachingRouteLocatorCustom)routeLocator).getRoutes(exchange);
        }
        return routes
