 ...
 * @author heelynn
 *
 * 为Route的metadata添加host metadata key, value是host name
 * 这样可以通过metadataHost来判断Route所属的域名，从而实现根据域名进行路由缓存
 *
 * {@link RouteLocator} that loads routes from a {@link HostRouteDefinitionRouteLocator}.
 * {@link HostRouteDefinitionRouteLocator} copy from {@link RouteDefinitionLocator}
 *
 * modify :
 *   更新下述方法，解析RouteDefinition的过程中，jinja2模板解析host metadata key, value是host name
 *   {@link HostRouteDefinitionRouteLocator#convertToRoute(RouteDefinition)}
 *
 */
public class HostRouteDefinitionRouteLocator implements RouteLocator {
     ...
     * add by renhailiang
     */
    public static final String METADATA_HOST = "metadataHost";
    ...
    public HostRouteDefinitionRouteLocator(RouteDefinitionLocator routeDefinitionLocator,
    ...
    private Route convertToRoute(RouteDefinition routeDefinition) {
        ...
        /*
         * get host , if does not exist, set ""
         * add by renhailiang
         */
        final String[] host = {""};
        routeDefinition.getPredicates().forEach(predicateDefinition -> {
            if (predicateDefinition.getName().equals("Host")) {
                host[0] = predicateDefinition.getArgs().entrySet().stream().findFirst().get().getValue();
                if (logger.isInfoEnabled()) {
                    logger.info("-- RouteDefinition " + routeDefinition.getId() + " matched Host " + host[0]);
                }
            }
        });

        return Route.async(routeDefinition).asyncPredicate(predicate).replaceFilters(gatewayFilters).metadata(METADATA_HOST, host[0]).build();
    }
        ...
        for (int i = 0; i < filterDefinitions.size(); i++) {
            ...
            Object configuration = this.configurationService.with(factory)
                    ...
                    .eventFunction((bound, properties) -> new FilterArgsEvent(
                            ...
                            HostRouteDefinitionRouteLocator.this, id, (Map<String, Object>) properties))
                    .bind();
            // @formatter:on
    ...
    private AsyncPredicate<ServerWebExchange> lookup(RouteDefinition route, PredicateDefinition predicate) {
        ...
        Object config = this.configurationService.with(factory)
                ...
                .eventFunction((bound, properties) -> new PredicateArgsEvent(
                        HostRouteDefinitionRouteLocator.this, route.getId(), properties))
                .bind();
        // @formatter:on
